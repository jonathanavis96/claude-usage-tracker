...
1. Joint fit (the route this file leads with). Nonnegative least squares of `delta_pct` on four
   columns -- Opus, Sonnet and Fable input-equivalent tokens (input + cache_write + 5x output,
   an assumption on the output multiplier) and total cache reads (an unweighted column here, not
   folded in at a fixed relative weight) -- over every accepted-capture-status stretch since the
   cut. B5 is `1 / coefficient on Opus`. An 80% row-bootstrap interval (10th-90th percentile,
   `SEED` fixed) is reported on the Fable/Opus coefficient ratio, and a rounding-only feasibility
   check (`tools/rounding_feasibility.py`'s linear programme, reads free) on the same rows says
   whether any nonnegative stationary rate is even consistent with whole-percent rounding.
2. The single-family anchor kept from the first pass of this file: every stretch where Opus is
   at least 90% of input + cache_write + 5x output on its own. Kept below as a paragraph, not a
   route this file solves further -- masterrig never runs Opus alone, so this threshold was
   always going to be sparse, and the joint fit is the route that uses every row.
...
family's own input-equivalent tokens taken at Opus's own per-token weight (an assumption: it
treats one Opus-equivalent token from any family as one Opus input token) and `read_charge` is
0.015 times the stretch's total cache reads across every model. `f` is solved once at B5's point
estimate (`1 / coef_opus` from the joint fit) and once at each of two rounding-implied edges,
built by refitting the joint fit against `delta_pct -+ windows` instead of `delta_pct`.
...
JOINT_FAMILIES = ("opus", "sonnet", "fable")   # the joint fit's four columns are these three + reads
...
def capture_status_counts(stretches_since_cut: list[dict]) -> dict[str, int]:
    """Every capture_status seen among the post-cut stretches, not just 'accepted'."""
    return dict(Counter(s.get("capture_status") for s in stretches_since_cut))


def accepted_since_cut(stretches_since_cut: list[dict]) -> list[dict]:
    return [s for s in stretches_since_cut if s.get("capture_status") == ACCEPTED_STATUS]
...
# --- Step 1, route A: the joint fit over every accepted-capture-status row since the cut -------

def joint_design(rows: list[dict], credits: dict) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Scaled [E_opus, E_sonnet, E_fable, R] columns, delta_pct target, windows pieces."""
    sums = [family_sums(s["tokens"], credits) for s in rows]
    x = np.array([[ie5(su[f]) / IE_UNIT for f in JOINT_FAMILIES] + [total_reads(su) / IE_UNIT]
                 for su in sums])
...
def joint_fit(rows: list[dict], credits: dict, seed: int = SEED, resamples: int = RESAMPLES) -> dict | None:
    """Nonnegative least squares of delta_pct on Opus/Sonnet/Fable input-equivalent tokens and
    reads, coefficients per raw token (column scale divided back out), plus a row bootstrap on
    the Fable/Opus coefficient ratio."""
    if not rows:
        return None
    ...
    rng = random.Random(f"{seed}/masterrig-joint-fit")
    n = len(rows)
    draws = []
    ...
    return {
        "n": n,
        "coef": dict(zip(JOINT_FAMILIES, coef_raw[:3])),
        "coef_reads": coef_raw[3],
...
def joint_fit_b5(fit: dict, rows: list[dict], credits: dict) -> dict | None:
    """B5 = 1 / coef_opus at the point estimate, and at each rounding-implied edge (refitting
    against delta_pct -+ windows instead of delta_pct)."""
    ...
    opus_lo, opus_hi = coef_lo[0], coef_hi[0]
...
def feasibility_reads_free(rows: list[dict], credits: dict) -> dict | None:
    """tools/rounding_feasibility.py's linear programme (reads free) on the same accepted rows:
    the smallest uniform extra percentage-point slack beyond whole-percent rounding at any
    nonnegative Opus/Sonnet/Fable rate. t=0 means the group is not excluded by rounding alone."""
    if not rows:
        return None
    x, y, pieces = joint_design(rows, credits)
    result = rounding_lp(x[:, :3], y, pieces)   # drop the reads column: reads free
...
def opus_anchor_rows(stretches: list[dict], credits: dict) -> list[dict]:
    """Rows behind the single-family anchor: every stretch where Opus is >= OPUS_DOMINANCE of
    total ie5. Kept for the null result; not the route step (ii) below solves against."""
    rows = []
    for s in stretches:
...
def show(joint: dict | None, joint_b5: dict | None, feas: dict | None, single_anchor: dict | None,
         env: dict) -> None:
    print(f"masterrig, stretches since {CUT.isoformat()} ({CUT_REASON})\n")
    print("Route A -- joint fit (nonnegative least squares, Opus/Sonnet/Fable input-equivalent "
          "tokens + cache reads, over every accepted-capture-status stretch since the cut):")
    if joint is None:
        print("  n=0: no accepted-capture-status stretch since the cut. No joint fit possible.\n")
    else:
        print(f"  n={joint['n']}  coef: opus={joint['coef']['opus']:.6g}  "
              f"sonnet={joint['coef']['sonnet']:.6g}  fable={joint['coef']['fable']:.6g}  "
              f"reads={joint['coef_reads']:.6g} (percentage points per raw token)")
        ratio = joint["fable_opus_ratio"]
        print(f"  Fable/Opus coefficient ratio: {ratio!r}"
        ...
        if joint_b5:
            ...
            print("  Opus coefficient is 0 or negative: no B5 from this fit.")
    if feas is None:
    ...
    if joint_b5 is None:
        print("No B5 from the joint fit either: the Fable solve below cannot run.")
        return
    if env["n"] == 0:
        print("No stretch is Fable-heavy enough (>= 50% of raw input+cache_write+output) to solve.")
        return
    print(f"Fable envelope (B5 from the joint fit): n={env['n']}, median f={env['median']:.4f} "
          f"Opus input tokens, 10th-90th percentile [{env['p10_p90'][0]:.4f}, "
          f"{env['p10_p90'][1]:.4f}]\n")
...
def main(argv: list[str] | None = None) -> int:
    ...
    accepted_rows = accepted_since_cut(all_since_cut)

    joint = joint_fit(accepted_rows, credits)
    joint_b5 = joint_fit_b5(joint, accepted_rows, credits) if joint else None
    feas = feasibility_reads_free(accepted_rows, credits)
    ...
    env = envelope(joint_b5, fable) if joint_b5 else {"n": 0, "median": None, "p10_p90": None, "rows": []}

    show(joint, joint_b5, feas, single_anchor, env)
    if a.json:
        a.json.write_text(json.dumps({
            ...
            "joint_fit": joint, "joint_fit_b5": joint_b5, "feasibility_reads_free": feas,
