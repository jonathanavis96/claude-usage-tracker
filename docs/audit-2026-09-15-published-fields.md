...
| `probe_accounts` | Distinct `account` tags on usable prose rows | Caveats: "Probes run on two Max 20x accounts (dave and jwork)" | Correct. "two" is hard-coded; the list is not. | Reflects all history, never stale. |
